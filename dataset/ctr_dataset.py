...
class Criteo2party(object):
    ...
    def __init__(self, data_dir, data_type, k, input_size, split_mode='equal'):
        ...
        train_pos = train[train['label'] == 1][:4800]
        train_neg = train[train['label'] == 0]
        test_pos = test[test['label'] == 1][:1200]
        test_neg = test[test['label'] == 0]
        train = pd.concat([train_pos, train_neg], axis=0)
        test = pd.concat([test_pos, test_neg], axis=0)

        print("[DEBUG] Criteo2party data shape:", data.shape)
        print("[DEBUG] Criteo2party train shape:", train.shape)
        print("[DEBUG] Criteo2party test shape:", test.shape)
        ...
        print("[DEBUG] Criteo2party pos y {}/{}".format(sum(self.y), len(self.y)))
        ...
        for i in range(len(sparse_features_list)):
            ...
            self.x.append(x)
        print(self.x[0].shape, self.x[1].shape)
        del data, train, test
...
class Avazu2party():
    ...
    def __init__(self, data_dir, data_type, k, input_size, split_mode='equal'):
        ...
        print("[DEBUG] train shape : {}, test shape : {}".format(train.shape, test.shape))
        ...
        print("[DEBUG] Avazu2party pos y {}/{}".format(sum(self.y), len(self.y)))
