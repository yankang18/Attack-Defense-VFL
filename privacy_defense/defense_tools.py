...
# Code is migrated from https://github.com/jiahuanluo/label-inference-attacks/blob/main/Code/possible_defenses.py
...
def multistep_gradient(tensor, bound_abs=3e-2, bins_num=12):
    if bound_abs is not None:
        max_min = 2 * bound_abs
    else:
        std, mean = torch.std_mean(tensor, unbiased=False, dim=0)
        max_min = 4 * std
...
class DPLaplacianNoiseApplyer():
    ...
    def laplace_mech(self, tensor):
        ...
        for i in range(noisy_mask.shape[0]):
            noisy_mask[i] = self.generate_lap_noise()
